...
| **Alternative Flows** | **A1: Invalid Email Format** - At step 4, if email format invalid, system displays error and highlights email field. Flow returns to step 3.<br><br>**A2: Weak Password** - At step 4, if password doesn't meet requirements, system displays detailed error. Flow returns to step 3.<br><br>**A3: Username/Email Already Exists** - At step 4 or 6, if username/email already taken, system displays error and highlights field. Flow returns to step 3.<br><br>**A4: Missing Required Fields** - At step 3 or 4, if any required field missing, system displays error and highlights missing fields. Flow returns to step 3.<br><br>**A5: Database/System Error** - At step 6-7, if database connection fails or system error occurs, system logs error and displays appropriate error message. Use case terminates. |
...
| **Alternative Flows** | **A1: Missing Credentials** - At step 2 or 3, if email/password missing, system displays error. Flow returns to step 1.<br><br>**A2: User Not Found** - At step 5, if user not found, backend returns "Invalid email or password". Flow returns to step 1.<br><br>**A3: Account Locked/Inactive** - At step 6, if account locked or inactive, backend returns appropriate error. Use case terminates.<br><br>**A4: Invalid Password** - At step 8, if password invalid, backend increments failed attempts (locks account if ≥ 4 attempts), returns error. Flow returns to step 1.<br><br>**A5: Database/Network Error** - At step 4 or any database operation, if connection fails, system displays "Network error. Please try again." Use case terminates. |
...
| **Alternative Flows** | **A1: No Session/Session Not Found** - At step 3, if session identifier not found or session record missing, backend continues logout (non-critical). Flow continues normally.<br><br>**A2: Database/Network Error** - At step 3 or 2, if database connection fails or network error occurs, backend logs error but returns success. Frontend continues with client-side logout (clears token). Flow continues to step 5. |
...
| **Alternative Flows** | **A1: AI Generation Failure** - At step 4, if AI API fails, system retries (max 3 attempts). If all fail, displays "AI service temporarily unavailable." Flow returns to step 1.<br><br>**A2: Validation Failure** - At step 5, if validation fails, Create Agent requests regeneration. If fails after 2 attempts, displays error. Use case terminates.<br><br>**A3: GitHub Push Failure** - At step 6, if GitHub push fails, system displays error and saves challenge locally. Flow returns to step 1.<br><br>**A4: Ambiguous/Invalid Request** - At step 3 or 4, if intent unclear or request too vague, system asks for clarification via chat. Flow returns to step 1.<br><br>**A5: API Rate Limit** - At step 4, if rate limit exceeded, system displays "AI service rate limit exceeded. Please try again later." Use case terminates. |
...
| **Alternative Flows** | **A1: Challenge Not Found** - At step 5-6, if challenge doesn't exist, system displays error with available challenges list. Use case terminates.<br><br>**A2: Validation Failure** - At step 9, if validation fails, Pre-Deploy Validator attempts automatic fixes using AI. If successful, flow continues. If fails, system displays error with detailed report. Use case terminates.<br><br>**A3: Container Build/Startup Failure** - At step 10, if build/startup fails, Deploy Agent analyzes logs, requests AI for fixes, and retries (max 3 attempts). If fails, system displays error with logs. Use case terminates.<br><br>**A4: Post-Deployment Validation Failure** - At step 11, if services not accessible, Post-Deploy Validator identifies issues. Deploy Agent fixes and restarts (max 3 attempts). If fails, displays error. Use case terminates.<br><br>**A5: Network/Guacamole Failure** - At step 10 or 12, if network allocation fails or Guacamole connection creation fails (max 2 retries), system displays error. For Guacamole failure, provides container IPs for manual access. Use case terminates. |
...
| **Alternative Flows** | **A1: Connection Timeout** - At step 4, if connection times out, system displays error. User can retry. Flow returns to step 1.<br><br>**A2: Container Unavailable** - At step 4, if container not running/accessible, system displays error suggesting redeployment. Use case terminates.<br><br>**A3: Invalid Credentials** - At step 3, if credentials invalid/expired, system displays error suggesting redeployment. Use case terminates.<br><br>**A4: Multiple Containers** - At step 5, if multiple containers exist, user can switch between them via Guacamole connection list. Flow continues normally.<br><br>**A5: Guacamole Service Unavailable** - At step 2, if service down, system displays error. Use case terminates. |
...
| **Alternative Flows** | **A1: Ambiguous Intent** - At step 4, if intent unclear, Questions Agent asks for clarification. Flow returns to step 1.<br><br>**A2: API Rate Limit** - At step 5, if rate limit exceeded, system displays "AI service busy. Please try again." and queues for retry. Flow returns to step 1.<br><br>**A3: Long Context** - At step 3, if conversation history very long, system includes only last 10-15 messages. Flow continues normally.<br><br>**A4: Challenge-Specific Questions** - At step 4, if user asks about specific challenge, Info Agent retrieves details from GitHub. Flow continues to step 5.<br><br>**A5: API/Database Error** - At step 5 or 2, if API service down or database connection fails, system displays error or logs and continues (history saved later). Use case terminates or continues. |
...
| **Alternative Flows** | **A1: Challenge Not Found** - At step 6, if challenge doesn't exist, Info Agent responds with error and suggests browsing challenges. Use case terminates.<br><br>**A2: Challenge Not Deployed** - If challenge not deployed, system displays "Not Deployed" status (no access URL/credentials) and suggests deployment. Flow continues to step 8.<br><br>**A3: Ambiguous Challenge Name** - At step 4, if name ambiguous or multiple matches, Info Agent asks user to clarify with options list. Flow returns to step 1.<br><br>**A4: GitHub Connection Failure** - At step 5, if GitHub cannot be accessed, system displays error. Use case terminates. |
...
| **Alternative Flows** | **A1: Change Password** - At step 3, if user changes password, system verifies current password, validates/hashes new password, updates in database, invalidates other sessions, displays success, and redirects to login. Flow ends.<br><br>**A2: Change Email** - At step 3, if email changed, system validates format and checks if already registered. If exists, displays error. If new, updates email. Flow continues to step 7.<br><br>**A3: Invalid Current Password** - At step 5 or A1, if current password incorrect, system displays error. Flow returns to step 3.<br><br>**A4: Weak Password/Password Mismatch** - At step 5, if new password weak or passwords don't match, system displays error with requirements. Flow returns to step 3.<br><br>**A5: No Changes** - At step 3, if no changes made, system displays "No changes to save". Flow continues to step 7.<br><br>**A6: Database Error** - At step 2 or 7, if database connection fails, system displays error. Use case terminates. |
...
| **Alternative Flows** | **A1: No Challenges** - At step 4, if no challenges exist, Info Agent responds suggesting to create one. Flow continues to step 7.<br><br>**A2: GitHub Connection Failure** - At step 4, if GitHub cannot be accessed, system displays error. Use case terminates.<br><br>**A3: Ambiguous Request** - At step 3, if intent unclear, Questions Agent asks for clarification. Flow returns to step 1. |
