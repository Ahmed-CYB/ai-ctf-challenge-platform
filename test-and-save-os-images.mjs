...
const { dbManager } = await import(`file://${join(ctfAutomationPath, 'db-manager.js')}`);
const { validateAndSaveOSImage, detectOSFamily } = await import(`file://${join(ctfAutomationPath, 'os-image-db-manager.js')}`);
...
async function testImage(imageName, tag, description, expectedPackageManager, expectedOSFamily) {
  ...
  try {
    ...
    try {
      await execAsync(`docker pull ${fullName}`, { timeout: 300000 });
    ...
    try {
      ...
      await execAsync(runCommand, { timeout: 30000 });
